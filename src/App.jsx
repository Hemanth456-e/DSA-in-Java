...
function Workflow({progress=76}){
 ...
 return <><div className="workflow">{steps.map((s,i)=><React.Fragment key={s[0]}><div className="step"><div className={"stepIcon "+s[2]}><s[3] size={17}/></div><strong>{s[0]}</strong><small>Federated process</small><em className={s[2]}>{s[1]}</em></div>{i<3&&<ChevronRight className="arrow"/>}</React.Fragment>)}</div><div className="progress"><div><b>Round 12 in progress</b><b>{progress}%</b></div><span><i style={{width:progress+"%"}}/></span></div></>
}
